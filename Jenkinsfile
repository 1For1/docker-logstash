...
node ('docker-cmd'){
    ...
    dockerlogin()
    dockerrmi("oneforone/logstash:${env.BRANCH_NAME}.${env.BUILD_NUMBER}")
    dockerbuild("oneforone/logstash:${env.BRANCH_NAME}.${env.BUILD_NUMBER}")

    stage 'Registry'
    slackSend color: 'green', message: "ORG: ${env.JOB_NAME} #${env.BUILD_NUMBER} - Pushing to Docker"
    dockerpush("oneforone/logstash:${env.BRANCH_NAME}.${env.BUILD_NUMBER}")
    ...
    switch ( env.BRANCH_NAME ) {
        case "master":
            ...
            // Erase
            dockerrmi('oneforone/logstash:latest')

            // Tag
            dockertag("oneforone/logstash:${env.BRANCH_NAME}.${env.BUILD_NUMBER}","oneforone/logstash:latest")

            // Push
            slackSend color: 'blue', message: "ORG: ${env.JOB_NAME} #${env.BUILD_NUMBER} - Pushing :latest"
            dockerpush('oneforone/logstash:latest')
            ...
            try {
                build '/OPS-Dev-Services/dev-service-logstash1-dev1'
                build '/OPS-Dev-Services/dev-service-logstash2-dev2'
                build '/OPS-Dev-Services/dev-service-logstash3-dev3'
                build '/OPS-Dev-Services/dev-service-logstash4-dev4'
                build '/OPS-Dev-Services/dev-service-logstash5-dev5'
            } catch (err) {
                echo "Build Error"
                echo err
            }
